...
export default function ApiDebug() {
  ...
  const testApiEndpoints = async () => {
    ...
    const endpoints = [
      { url: "/home", method: "GET" },
      { url: "/product-page/ba3", method: "GET" },
    ...
    for (const endpoint of endpoints) {
      ...
      console.log(`🔄 Testing API: ${endpoint.method} ${fullUrl}`);
      ...
      try {
        ...
        const index = newApiCalls.findIndex(
          (call) => call.endpoint === endpoint.url && call.status === "pending"
        );
  ...
  return (
    <div
      className="fixed bottom-0 right-0 z-50 w-96 max-w-full"
          ...
          <div className="p-3 max-h-96 overflow-auto">
            {apiCalls.length === 0 ? (
              <p className="text-gray-400 text-sm">
                No API calls recorded yet. Click the refresh button to test
                endpoints.
              </p>
            ) : (
              <div className="space-y-3">
                {apiCalls.map((call, index) => (
                  <div
                    ...
                    {call.requestDetails && (
                      <div className="mt-1 text-xs bg-gray-900 p-1 rounded overflow-auto max-h-20">
                        <div>URL: {call.requestDetails.url}</div>
                        <div>
                          Headers: {JSON.stringify(call.requestDetails.headers)}
                        </div>
                      </div>
                    )}
